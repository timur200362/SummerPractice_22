    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button?.setOnClickListener{
            ...
            if(name.length<0 || name.length>50 || height>0 || height<250 || weight>0 || weight<250 || age>0 || age<150){
                res.setText("Данные введены некорректно")
            }
            else{
                //val res =findViewById<TextView>(R.id.result)
                //res.text=((height*5)-(age*6.8)+(weight*13.7)+66).toInt()
                val result:Long=Math.round((height*5)-(age*6.8)+(weight*13.7)+66)
                res.setText("$name Ваша норма каллорий:$result")
